    ...
    try:
        ...
        if feedbacks:
            for feedback in feedbacks:
                text_preview = feedback[1][:50] + "..." if feedback[1] and len(feedback[1]) > 50 else feedback[1]
                print(f"  ID: {feedback[0]}, text: '{text_preview}', user_id: {feedback[2]}, product_id: {feedback[3]}")
        else:
            print("  Не найдено")
        ...
        if payments:
            for payment in payments:
                details_preview = payment[1][:50] + "..." if payment[1] and len(payment[1]) > 50 else payment[1]
                print(f"  ID: {payment[0]}, details: '{details_preview}', user_id: {payment[2]}, product_id: {payment[3]}")
        else:
            print("  Не найдено")
